...
const Dashboard = () => {
  ...
  const fetchBalance = async () => {
    try {
      ...
      const response = await fetch(`http://54.86.117.121:8083/api/dashboard/balance/${currentUser.id}`, {
        method: "GET",
        headers: {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      const response = await fetch(`http://54.86.117.121:8083/api/transactions/${currentUser.id}`, {
        method: "GET",        
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${localStorage.getItem("token")}`,
